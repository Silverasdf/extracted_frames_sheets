...
the tabular data, so that the model will give a more detailed answer. This should not involve too much work. For the image data, my current idea is to use Facebook's Segment Anything model to only take the important parts of the image to send in. After this, we can go through a normalization step and send it into the model to fine tune. These preprocessing techniques should, theoretically, be quicker, as we will need to preprocess any data that is given in real time. Another approach is to only preprocess the images
to fine tune them, but then leave the image sequences alone as they are during evaluation.

One other consistent issue I have the is lack of data to fine tune on. I have been working with Dr. Yangsong Gu to get a better dataset, but this dataset is focused on the shoulder lane of highways, rather than full accidents. However, if we are able to do preprocessing and get good results from a model fine-tuned on this dataset, we can then use the same methodology on my dataset, as long as I can get more data. I believe that I can easily get more data by pulling it from YouTube, and then I can use the new version of Gemini, Gemini 2.5, to take the videos and label them properly, with some prompt engineering. After that, I could speed up my evaulation accuracy with something like the ROUGE metric, rather than building an accuracy matrix, which takes a lot of time to make.
